...
const CardTopicUi = ({ ...props }: Props) => {
  return (
    <>
      <h1 className="text-white text-2xl font-bold mb-4 text-left">{props.title}</h1>
      <div className="grid grid-cols-7 gap-4">
        {props.data.map((item, index) => (
          <div key={item._id} className={`${item.gradient} group hover:shadow-xl transition-all duration-300 flex-col items-start pt-16 pb-5 px-6 rounded-lg cursor-pointer overflow-hidden relative backdrop-blur-sm bg-opacity-90 flex justify-end`}>
        ...
        {props.more ? (
          <div className={`bg-[#535666] group hover:shadow-xl transition-all duration-300 flex-col rounded-lg cursor-pointer overflow-hidden relative backdrop-blur-sm bg-opacity-90 flex justify-center items-center`}>
            <div className="absolute inset-0 opacity-0 group-hover:opacity-10 bg-white transition-opacity duration-300"></div>
